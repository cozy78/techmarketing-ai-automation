...
function initCustomSelects() {
    document.querySelectorAll('select.input-field').forEach(function(selectElement) {
        ...
        // Schließe alle anderen Dropdowns und entferne auch die 'open-upward' Klasse
            document.querySelectorAll('.custom-select').forEach(s => 
                s.classList.remove('open', 'open-upward')); //
        ...
        wrapper.classList.remove('active')
            ...
            if (!wasOpen) {
                ...
                const triggerRect = trigger.getBoundingClientRect(); // Position und Größe des Triggers
                const optionsHeight = options.scrollHeight; // Tatsächliche Scroll-Höhe der Optionen
                const spaceBelow = window.innerHeight - triggerRect.bottom; // Platz unter dem Trigger
                const spaceAbove = triggerRect.top; // Platz über dem Trigger
               
                // Prüfe, ob genug Platz unter dem Trigger ist
                if (optionsHeight > spaceBelow && spaceAbove > optionsHeight) {
                    customSelect.classList.add('open-upward'); //
                } else {
                    customSelect.classList.remove('open-upward'); // Sicherstellen, dass es entfernt wird, wenn genug Platz ist oder es nicht nach oben passt
                }
            }
        });
    }); 
